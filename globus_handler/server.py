...
class GCThread(threading.Thread):
    ...
    def run(self):
        ...
        _runGCCommand(
            "%s/globusconnectpersonal" % self.gcDir,
            "-start",
            "-restrict-paths",
            "rw%s" % self.globusRoot,
            "-shared-paths",
            self.globusRoot,
            "-dir",
            self.confDir,
...
class Server:
    def __init__(self, clients):
        ...
        self.globusRoot = Setting().get(
            PluginSettings.GLOBUS_ROOT_PATH, "/tmp/wt-globus"
        )
    ...
    def _getEndpointDataByKey(self, key):
        ...
        (endpointIds, endpointNames) = self._maybeConvertSettings(
            endpointIds, endpointNames
        )
    ...
    def _createUserEndpoint(self, user):
        ...
        if failure is not None:
            ...
            raise Exception(
                "Could not set ACL rule on shared endpoint for user %s: %s"
                % (userName, failure)
            )
    ...
    def _getGCDir(self):
        return self.clients._getGlobusSetting(
            GlobusEnvironmentVariables.GLOBUS_CONNECT_DIR,
